...
Strongest when actually enforced by a dependency-check in CI, a module boundary the build rejects, or a type that makes the wrong dependency unrepresentable. A constraint that lives only in prose is a project rule wearing a stronger name, so if it can't be enforced, route it to project rule and be honest that it depends on people remembering.
