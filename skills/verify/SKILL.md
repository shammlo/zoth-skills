...
This is a QA discipline externalized as a skill, not a generic "run tests" reminder. The bar: a "done" claim backed by both test count and a description of what was actually executed at runtime — e.g. "135 tests green + typecheck clean" is the *right shape* of evidence, and this skill exists to make that the floor for every task, not just the ones where it happened to be top of mind.
...
Before implementation begins, write down — in a sentence or two — what observable behavior proves this is finished. Not "the endpoint exists" but a specific, checkable claim — e.g. "a request from an unauthorized tenant returns 403 and no row is written." If you can't state this up front, the task isn't scoped enough to verify later.
